...
async function handleScheduled(event, env) {
  ...
  try {
    ...
    const response = await fetch(liveMatchesUrl, {
      ...
      cf: { cacheTtl: 60 }
      // Cache for 60 seconds on Cloudflare
    });
    if (!response.ok) {
...
async function processMatch(matchSummary, KV2, POKE_WEBHOOK) {
  ...
  try {
    const response = await fetch(matchUrl, {
      headers: FETCH_HEADERS,
      cf: { cacheTtl: 30 }
      // Cache for 30 seconds on Cloudflare
    });
    if (!response.ok) {
...
// .wrangler/tmp/bundle-EW93fH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-EW93fH/middleware-loader.entry.ts
